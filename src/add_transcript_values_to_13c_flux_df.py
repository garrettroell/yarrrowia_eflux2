...
def add_transcript_values_to_13c_flux_df(central_flux_df, genome_scale_df, trans_LB_column_name, trans_UB_column_name):
    ...
    for _, row in central_flux_df.iterrows():
        ...
        if pd.isna(row['reaction_ids']):
            ...
            trans_LB_list.append('')
            trans_UB_list.append('')
        else:
            # Add the flux value for each row to the column values list
            reaction_ids = row['reaction_ids']
            
            # get the lower and upper bounds
            trans_LB, trans_UB = reaction_ids_to_bounds(genome_scale_df, reaction_ids, trans_LB_column_name, trans_UB_column_name)

            # add the lower and upper bounds to the lists
            trans_LB_list.append(trans_LB)
            trans_UB_list.append(trans_UB)

    # add the column to the dataframe
    updated_df[trans_LB_column_name] = trans_LB_list 
    updated_df[trans_UB_column_name] = trans_UB_list 

    return updated_df

def reaction_ids_to_bounds(genome_scale_df, reaction_ids, trans_LB_column_name, trans_UB_column_name):
    total_lower_bound = 0
    total_upper_bound = 0
    for x in [x.strip(' ') for x in reaction_ids.split(' or ')]:
        and_split = [y.strip(' ') for y in x.split(' and ')]
        lower_bounds = [reaction_id_to_bounds(v, genome_scale_df, trans_LB_column_name, trans_UB_column_name)[0] for v in and_split]
        upper_bounds = [reaction_id_to_bounds(v, genome_scale_df, trans_LB_column_name, trans_UB_column_name)[1] for v in and_split]
        total_lower_bound += min(lower_bounds)
        total_upper_bound += max(upper_bounds)
            
    return total_lower_bound, total_upper_bound

def reaction_id_to_bounds(reaction_id, genome_scale_df, trans_LB_column_name, trans_UB_column_name):
    ...
    lower_bound = reaction_row[trans_LB_column_name]
    upper_bound = reaction_row[trans_UB_column_name]
